<section id="hero" class="flex flex-col items-center gap-8 py-16 text-center sm:py-20" data-fade-in>
    ...
    <div class="space-y-3">
        ...
        @if ($user->phone)
            <p class="inline-flex items-center gap-1.5 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                <a href="https://wa.me/{{ preg_replace('/\D/', '', $user->phone) }}" target="_blank" rel="noopener noreferrer" class="transition-colors hover:text-amber-600 dark:hover:text-amber-400">{{ $user->phone }}</a>
            </p>
        @endif
    </div>
    ...
    <a
        href="{{ route('cv') }}"
        class="inline-flex items-center gap-2 rounded-md bg-amber-600 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-amber-500 print:hidden dark:bg-amber-500 dark:hover:bg-amber-400"
    >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
